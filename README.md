...
- `매물목록조회`는 기존 목록이 있으면 기존 매물 순위 재조회와 최신 목록 수집을 병행하고, API 결과 기준으로 신규 매물을 추가하고 종료된 매물을 삭제한 뒤 신규 매물 순위까지 조회합니다. 옆의 `순위조회`는 체크된 매물이 있으면 체크 항목만, 없으면 현재 목록 전체를 재조회합니다.
...
- `Update`: GitHub Releases 버전 확인 설정
  ...
  - `ReleaseTagPrefix`: 이 프로그램의 릴리스를 구분할 태그 접두사. 기본 예시는 `NaverPropertyRanking-v`
...
[release.yml](.github/workflows/release.yml)은 `NaverPropertyRanking-v0.0.2` 같은 태그가 푸시되면 다음 작업을 자동 실행합니다.

1. .NET 8 복원·빌드·스모크 테스트
2. Windows x64 단일 EXE 게시
3. 배포용 설정의 현재 버전을 태그 버전으로 변경
4. GitHub Release 생성
5. `NaverPropertyRanking.exe`를 압축하지 않고 Release 자산으로 직접 업로드

```powershell
git tag NaverPropertyRanking-v0.0.2
git push origin NaverPropertyRanking-v0.0.2
```

배포할 때 `appsettings.json > Update.CurrentVersion`과 Release 태그 버전을 동일하게 맞춥니다. 예를 들어 기존 버전이 `0.0.1`이면 새 EXE를 `0.0.2`로 빌드하고 `NaverPropertyRanking-v0.0.2` Release에 `NaverPropertyRanking.exe`라는 이름으로 올립니다.
